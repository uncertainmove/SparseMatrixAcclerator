...
void Backend_Source_Core_Single(int Core_ID,
    ...
    static queue_container<int> queue_container[CORE_NUM];
    static value_container<int> Front_V_container[CORE_NUM],Om2_V_container[CORE_NUM],Rd_V_container[CORE_NUM];


    if(clk == 0){
        if( Core_ID == 0 )
        for(int i = 0 ; i<CORE_NUM ; i++){
            queue_container[i].combine({&update_v_id_buffer[i],&pull_first_flag_buffer[i]});
        }
        Front_V_container[Core_ID].combine({&Om1_Recv_Update_V_ID, &Om1_Recv_Update_V_Pull_First_Flag});
        Om2_V_container[Core_ID].combine({Om2_Send_Update_V_ID, Om2_Send_Update_V_Pull_First_Flag});
        Rd_V_container[Core_ID].combine({Rd_Vertex_BRAM_Addr, Rd_Vertex_BRAM_Valid});
    }


    if (rst_rd) {
        Rd_V_container[Core_ID].assign({0,0});
        ...
        if (Om1_Recv_Update_V_DValid && Om1_Recv_Push_Flag == 0) {
            ...
            if (Om1_Recv_Update_V_ID != Om1_Recv_Update_V_Value) {
                Rd_V_container[Core_ID].assign({Om1_Recv_Update_V_Value,1});
            } else {
                Rd_V_container[Core_ID].assign({-2,1});
            }

            queue_container[Core_ID].push(Front_V_container[Core_ID]);
        }
        else {
            Rd_V_container[Core_ID].assign({0,0});
        }
